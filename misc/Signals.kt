...
/**
 * A MutableMap used specifically for managing Lambdas Specified Type is used as the input for
 * argument for listener Lambdas
 */
data class BiSignal<Type, Type2>(
    val listeners: MutableMap<String, (Type, Type2) -> Unit> = mutableMapOf()
) {
    /**
     * Adds a listener with the given name. The function will be run whenever the parent object
     * calls the update function
     *
     * @param name String to denote the name of the listener, used to remove specific listeners
     *   later on
